...
CREATE TABLE IF NOT EXISTS `fiasdb`.`address_object` (
  ...
  `address_object_operation_status` TINYINT NOT NULL DEFAULT 20 COMMENT 'Статус действия над записью – причина появления записи.',
...
CREATE TABLE IF NOT EXISTS `fiasdb`.`operation_statuses` (
  `operation_statuses_id` TINYINT NOT NULL COMMENT 'Идентификатор статуса (ключ)',
...
CREATE TABLE IF NOT EXISTS `fiasdb`.`actual_statuses` (
  `active_status_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор статуса (ключ)',
  `active_status_name` VARCHAR(100) NOT NULL COMMENT 'Наименование\n0 – Не актуальный\n1 – Актуальный (последняя запись по адресному объекту)',
...
CREATE TABLE IF NOT EXISTS `fiasdb`.`center_status` (
  `center_status_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор статуса',
...
CREATE TABLE IF NOT EXISTS `fiasdb`.`structure_status` (
  `structure_status_id` SMALLINT NOT NULL AUTO_INCREMENT,
